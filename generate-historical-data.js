// Generate historical data (2015-2018) from the 2018 baseline data
// This creates realistic time-series data for forecasting demonstration
...
// - Growth trend: 5-15% year-over-year growth
// - Add some noise/variation
// - Some companies might not exist in earlier years
function generateHistoricalData() {
  ...
  data2018.rows.forEach(row => {
    ...
    for (let y = startYear; y <= 2018; y++) {
      // Calculate revenue with backwards projection
      const yearsBack = 2018 - y;
      const growthFactor = Math.pow(1 + baseGrowthRate, yearsBack);

      // Add some random noise (±10%)
      const noise = 0.9 + Math.random() * 0.2;
      const revenue = Math.round((revenue2018 / growthFactor) * noise);

      allRows.push([acc_code, cpyname, country, territory, y, revenue]);
    }
  });

  return {
    headers: data2018.headers,
    types: data2018.types,
    rows: allRows.sort((a, b) => {
      // Sort by year, then territory, then company
...
fs.writeFileSync('historical-data.json', JSON.stringify(historicalData, null, 2));
console.log(`Generated ${historicalData.rows.length} rows of historical data (2015-2018)`);
